# Mein Scope — Features Page Copy (Draft 1)
...
- Validation on required fields — nothing critical slips through
...
- Media attaches directly to the patient record — no manual matching
- *Note: video recording currently saves in WebM format. MP4 export is planned but not yet available.*

*Annotation:* Calling out the WebM-only limitation directly (rather than hiding it) matches the brief's instruction not to overclaim — and it's the kind of detail a technical evaluator will check anyway, so naming it builds trust instead of costing it.
...
## Section 4 — Runs without the internet

**Header:** Offline isn't a fallback mode — it's how it's built
...
- Installable as a Progressive Web App directly from the browser — no app store, no IT deployment project
